...
public class PinataListener implements Listener {
    ...
    public void onPinataHit(EntityDamageByEntityEvent event) {
        ...
        if (currentHits <= 0) {
            ...
            actionHandler.process(player, pinataConfig.events.hit.rewards.values(), cmd -> plugin.getMessageUtils().parsePinataPlaceholders(pinata, cmd));
    ...
    private void handlePinataDeath(LivingEntity pinata, Player player, PinataConfiguration pinataConfig) {
        ...
        actionHandler.process(player, pinataConfig.events.lastHit.rewards.values(), cmd -> plugin.getMessageUtils().parsePinataPlaceholders(pinata, cmd));
        ...
        actionHandler.process(player, pinataConfig.events.death.rewards.values(), cmd -> plugin.getMessageUtils().parsePinataPlaceholders(pinata, cmd));
