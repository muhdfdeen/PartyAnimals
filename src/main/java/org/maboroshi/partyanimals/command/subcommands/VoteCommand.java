...
public class VoteCommand {
    ...
    private int handleCheck(CommandContext<CommandSourceStack> ctx) {
        ...
        Bukkit.getAsyncScheduler().runNow(plugin, (task) -> {
            ...
            messageHandler.send(sender, "<prefix> <white>" + targetName + "</white> has <aqua>" + votes + "</aqua> votes.");
        });
    ...
    private int handleModify(CommandContext<CommandSourceStack> ctx, String action) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            if (finalChange == 0) {
                messageHandler.send(sender, "<prefix> <yellow>No changes made. " + targetName + " already has " + currentVotes + " votes.");
                return;
            }
            ...
            messageHandler.send(sender, "<prefix> <gray>Old: <yellow>" + currentVotes + "</yellow> -> New: <aqua>" + newTotal + "</aqua>");
        });
